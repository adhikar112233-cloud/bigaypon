 ...
 const refreshUser = useCallback(async () => {
  ...
  const refreshAllData = useCallback(async () => {
      if (user && platformSettings) {
        ...
        // Fix: Replace call to non-existent 'getInfluencers' with 'getInfluencersPaginated' and implement pagination logic.
  ...
  const handleStartLiveHelp = async () => {
    ...
    const staffUser = allUsers.find(u => u.id === staffUserId);
    if (!staffUser) {
        alert("The assigned live help agent is currently unavailable.");
        return;
    }
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case View.ADMIN:
        // Fix: Pass allUsers, allTransactions, and allPayouts to AdminPanel to resolve prop type errors.
