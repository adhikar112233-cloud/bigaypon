...
// Fix: The Timestamp type should be imported from 'firebase/firestore'.
...
// Fix: Import Timestamp from firebase/firestore to fix type errors
...
const MyAdBookingsPage: React.FC<MyAdBookingsPageProps> = ({ user, platformSettings, onStartChat, onInitiateRefund }) => {
    ...
    const handlePaymentSuccess = () => {
        if (payingRequest) {
            handleUpdate(payingRequest, { status: 'in_progress', paymentStatus: 'paid' });
        }
    ...
    return (
        <div className="space-y-6">
            ...
            {requests.length === 0 ? (
                <div className="text-center py-10 bg-white rounded-lg shadow"><p className="text-gray-500">You have no active ad bookings. Explore opportunities on the discovery pages.</p></div>
            ) : (
