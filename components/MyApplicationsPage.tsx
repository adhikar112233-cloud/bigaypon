        ...
        <div className="space-y-8">
            ...
            {applications.length === 0 ? (
                ...
                <div className="space-y-8">
                    {active.length > 0 && (
                        <div>
                             <h2 className="text-2xl font-bold text-gray-800 mb-4">Active Applications</h2>
                             <div className="bg-white shadow-lg rounded-2xl overflow-hidden"><ul className="divide-y divide-gray-200">
                                {active.map(app => <ApplicationItem key={app.id} app={app} renderStatusInfo={renderStatusInfo} renderActions={renderActions} />)}
                            </ul></div>
                        </div>
                    )}
                     {pending.length > 0 && (
                        <div>
                             ...
                             <div className="bg-white shadow-lg rounded-2xl overflow-hidden"><ul className="divide-y divide-gray-200">
                                {pending.map(app => <ApplicationItem key={app.id} app={app} renderStatusInfo={renderStatusInfo} renderActions={renderActions} />)}
                            </ul></div>
                        </div>
                    )}
                     {archived.length > 0 && (
                        <div>
                             <h2 className="text-2xl font-bold text-gray-800 mb-4">Archived & Completed</h2>
                             <div className="bg-white shadow-lg rounded-2xl overflow-hidden"><ul className="divide-y divide-gray-200">
                                {archived.map(app => <ApplicationItem key={app.id} app={app} renderStatusInfo={renderStatusInfo} renderActions={renderActions} />)}
                            </ul></div>
                        </div>
                    )}
                </div>
