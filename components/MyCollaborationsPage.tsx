...
const MyCollaborationsPage: React.FC<MyCollaborationsPageProps> = ({ user, platformSettings, onViewProfile, onStartChat, onInitiateRefund }) => {
    ...
    const handlePaymentSuccess = () => {
        if (payingRequest) {
            handleUpdate(payingRequest.id, { status: 'in_progress', paymentStatus: 'paid' });
        }
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex space-x-2 p-1 bg-gray-50 rounded-lg border">
                 ...
                 <button onClick={() => setFilter('archived')} className={`px-4 py-2 text-sm font-semibold rounded-lg ${filter === 'archived' ? 'bg-indigo-600 text-white' : 'text-gray-600'}`}>Archived</button>
            </div>
