...
import { User, SupportTicket, TicketReply, SupportTicketStatus, Attachment, PlatformSettings } from '../types';
...
const UserSupportPage: React.FC<{ user: User, platformSettings: PlatformSettings, onStartLiveHelp: () => void; }> = ({ user, platformSettings, onStartLiveHelp }) => {
    ...
    const isLiveHelpEnabled = !!platformSettings.liveHelpStaffId;
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={onStartLiveHelp}
                        disabled={!isLiveHelpEnabled}
                        title={isLiveHelpEnabled ? "Chat with a support agent now" : "Live help is currently unavailable"}
                        className="px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-teal-400 to-indigo-600 rounded-lg shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Start Live Chat
                    </button>
                </div>
